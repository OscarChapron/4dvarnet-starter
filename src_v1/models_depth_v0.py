...
class Lit4dVarNet_depth(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        # Assume batch.input has shape (batch_size, depth, time, lat, lon)
        #print('batch_forwrd')
        #print(batch.input.size())
        #outputs = []
        #for depth_level in range(batch.input.shape[2]):
        #    batch_depth = batch._replace(input=batch.input[:, :, depth_level, :, :], tgt=batch.tgt[:, :, depth_level, :, :])
        #    print('batch_depth_forwrd')
        #    print(batch_depth.input.size())
        #    output_depth = self.solver(batch=batch_depth)
        #    outputs.append(output_depth)
        ## Combine the outputs for all depth levels
    ...
    def step(self, batch, phase=""):
        ...
        for depth_level in range(batch.input.shape[2]):
            ...
            if self.training and batch.tgt.isfinite().float().mean() < 0.9:
                return None, None
            loss, out = self.base_step(batch_depth, phase)
            outputs.append(out.unsqueeze(1))  
        
            #grad_loss = self.weighted_mse( kfilts.sobel(out) - kfilts.sobel(batch.tgt), self.rec_weight)
            prior_cost = self.solver.prior_cost(self.solver.init_state(batch, out))
            #self.log( f"{phase}_gloss", grad_loss, prog_bar=True, on_step=False, on_epoch=True)
            #training_loss_temp = 2 * loss + 20 * prior_cost + 5 * grad_loss
            training_loss_temp = 2 * loss + 20 * prior_cost
            #training_loss = 50 * loss + 1000 * grad_loss + 1.0 * prior_cost
            training_loss += training_loss + training_loss_temp

        return training_loss, torch.cat(outputs, dim=1)
    ...
    def test_step(self, batch, batch_idx):
        ...
        for depth_level in range(batch.input.shape[2]):
            batch_depth = batch._replace(input=batch.input[:, :, depth_level, :, :], tgt=batch.tgt[:, :, depth_level, :, :])
            mask = (torch.rand(batch_depth.input.size()) > self.sampling_rate).to('cuda:0')
            masked_input = batch_depth.input.clone()
            masked_input[mask] = float('nan')
            batch_input_clone = batch.input.clone()
            batch_depth = batch_depth._replace(input = masked_input)
            out = self(batch=batch_depth)
            m, s = self.norm_stats
    ...
    def on_test_epoch_end(self):
        ...
        self.test_data = self.test_data.view(*self.trainer.test_dataloaders.dataset.shape)
